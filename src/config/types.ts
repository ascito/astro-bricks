...
export interface SiteConfig {
  ...
  brand: string;
  /** Domaine sans protocole. Ex : `'saunaguide.fr'`. */
  domain: string;
  /** URL canonique avec protocole. Ex : `'https://saunaguide.fr'`. */
  siteUrl: string;
  ...
  // ─── Affiliation Amazon ───
  /** Tag d'affiliation Amazon Associates. Ex : `'saunaguide-21'`. */
  amazonTag: string;
 ...
 */
export interface ResolvedSiteConfig extends Required<Omit<SiteConfig, 'social' | 'overrides' | 'fontDisplay' | 'foundingYear' | 'publisherAddress'>> {
  social: SocialLinks;
  overrides: ComponentOverrides;
  fontDisplay: string;
  foundingYear: number;
